...
fileprivate func main() {
    ...
    // Based on the FORTRAN I approach from https://en.wikipedia.org/wiki/Operator-precedence_parser#Alternative_methods
    func reparen(_ input: [String]) -> [String] {
        var result: [String] = ["(", "("]
        for c in input {
            if c == "+" {
                result += [")", "+", "("]
    ...
    for line in lines {
        ...
        let root = parse(reparen(line.replacingOccurrences(of: " ", with: "").map(String.init)).reversed())
        answer += root.value()
    }
    print("answer", answer)
}
